...
public class MusicBandService {
    ...
    public MusicBand getBandWithMinDescription() {
        List<MusicBand> bands = musicBandRepository.findAllOrderByDescriptionLengthAsc();
        if (bands.isEmpty()) return null;
        return bands.get(0);
    }

    public Long countMusicBandWithStudioGreaterThan(Long studioId) {
        return musicBandRepository.countByStudioIdGreaterThan(studioId);
    }

    public List<MusicBand> getBandsWithStudioGreater(Long studioId) {
        return musicBandRepository.findAll().stream()
                .filter(band -> band.getStudio() != null && band.getStudio().getId() > studioId)
                .toList();
    }

    public NominationResponseDTO awardBestBand(Long bandId, MusicGenre selectedGenre) {
        MusicBand band = musicBandRepository.findById(bandId)
                .orElseThrow(() -> new NotFoundException("Music band was not found"));

        // Check nếu genre không khớp
        if (!band.getGenre().equals(selectedGenre)) {
            throw new IllegalArgumentException("Selected genre does not match the band's genre");
        }

        Nomination nomination = new Nomination();
        nomination.setGenre(selectedGenre);
        nomination.setNominationTime(LocalDateTime.now());
        nominationRepository.save(nomination);

        return new NominationResponseDTO(band.getId(), selectedGenre, nomination.getNominationTime());
    }
